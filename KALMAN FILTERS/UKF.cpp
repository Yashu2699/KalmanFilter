...
std::vector<double> generateSigmaWeights(unsigned int numStates)
{
    ...
    double wi = 0.5 / (lambda + numStates);
...
VectorXd lidarMeasurementModel(VectorXd aug_state, double beaconX, double beaconY)
{
    ...
    double zhat_range = sqrt(diff_x*diff_x + diff_y*diff_y) + range_noise;
    double zhat_theta = atan2(diff_y, diff_x) - psi + theta_noise;
...
void KalmanFilter::handleLidarMeasurement(LidarMeasurement meas, const BeaconMap& map)
{
    if (isInitialised())
    {
        ...
        if (meas.id != -1 && map_beacon.id != -1) // Check that we have a valid beacon match
        {
            ...
            // Augmented sigma points transformation using measurement model
...
void KalmanFilter::predictionStep(GyroMeasurement gyro, double dt)
{
    if (isInitialised())
    {
        ...
        // ENTER YOUR CODE HERE
         ...
         // Generate augmented sigma points and sigma weights
         std::vector<VectorXd> sigma_points = generateSigmaPoints(x_aug, P_aug);
         std::vector<double> sigma_weights = generateSigmaWeights(n_aug);

         // Predict new state for augmented sigma points
